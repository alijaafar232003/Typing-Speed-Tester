...
function startTimer(){
    var constantTimer = leadingZero(timer[0]) + ":"+leadingZero(timer[1]) + ":"+leadingZero(timer[2]);
    theTimer.innerHTML = constantTimer; 
    timer[3]++;
...
// Start the timer:
function start () {
    let textAddedLength= testArea.value.length;
    if (textAddedLength === 0 && !timerRunning){
        timerRunning =true;
